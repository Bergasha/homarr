...
export const BoardSelectionToolbar = () => {
  ...
  return (
    // Portalled to the document body - a `filter`/`backdrop-filter` on an ancestor (e.g. from a
    // board's custom CSS) creates a new containing block for `position: fixed` descendants,
    // which would otherwise anchor this toolbar to that ancestor's box instead of the viewport.
    <Portal>
      <Paper
        withBorder
        shadow="xl"
